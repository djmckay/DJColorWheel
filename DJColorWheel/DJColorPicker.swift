...
public protocol DJColorPickerDelegate
{
    func colorSelectionChanged(sender: Any?, selectedColor color: UIColor)
}

open class DJColorPicker: UIView, ColorWheelDelegate, BrightnessViewDelegate {
    ...
    open var delegate: DJColorPickerDelegate?
    ...
    open func setViewColor(_ color: UIColor) {
        ...
        if (!ok) {
            print("DJColorPicker: exception <The color provided to DJColorPicker is not convertible to HSV>")
        }
        self.hue = hue
    ...
    func hueAndSaturationSelected(_ hue: CGFloat, saturation: CGFloat) {
        ...
        if let delegate = self.delegate {
            delegate.colorSelectionChanged(sender: self, selectedColor: self.color)
        }
    }
